# gat_edge_split_train_unified.py
# Edge-split training:
#   V_mp = V (all nodes)
#   E_mp = E_train (train edges only)
#   E+   = E_train
...
def build_or_load_node_id_map(paths, cache_path):
...
# -------------------------
def main():
    parser = argparse.ArgumentParser(description="Edge-split GAT training with degree-matched negatives.")
    parser.add_argument("--split_dir", type=str, required=True, help="Directory containing edge_split_{train,val,test}.edgelist.gz")
    ...
    TRAIN_EDGES_PATH = os.path.join(args.split_dir, "edge_split_train.edgelist.gz")
    VAL_EDGES_PATH   = os.path.join(args.split_dir, "edge_split_val.edgelist.gz")
    TEST_EDGES_PATH  = os.path.join(args.split_dir, "edge_split_test.edgelist.gz")

    NODEMAP_CACHE = os.path.join(args.out_dir, "node_id_map_edge_split.pkl")
    ...
    MODEL_OUT = os.path.join(args.out_dir, f"gat_edge_model_{RUN_TAG}.pt")
    EMB_OUT   = os.path.join(args.out_dir, f"gat_edge_embeddings_by_name_{RUN_TAG}.pkl")
    NODEMAP_OUT = os.path.join(args.out_dir, f"node_id_map_edge_split_{RUN_TAG}.pkl")

    print("Building/loading node map...")
    node2id = build_or_load_node_id_map(
        [TRAIN_EDGES_PATH, VAL_EDGES_PATH, TEST_EDGES_PATH],
        NODEMAP_CACHE
    )
    ...
    print("Loading train edges for message passing graph (E_mp = E_train)...")
    train_edge_index = load_edges_all(TRAIN_EDGES_PATH, node2id)
    train_edge_index = to_undirected(train_edge_index, num_nodes=num_nodes)
    print(f"MP edges (undirected) = {train_edge_index.size(1):,}")

    data = Data(edge_index=train_edge_index, num_nodes=num_nodes).to(DEVICE)
    pos_pool = train_edge_index.to(DEVICE)  # E+ = E_train

    print("Building observed edge set (full network) and degree sampler...")
    val_e  = load_edges_all(VAL_EDGES_PATH, node2id)
    test_e = load_edges_all(TEST_EDGES_PATH, node2id)
    full_e = torch.cat([train_edge_index, to_undirected(val_e, num_nodes=num_nodes),
                        to_undirected(test_e, num_nodes=num_nodes)], dim=1) if val_e.numel() > 0 or test_e.numel() > 0 else train_edge_index
    ...
    print("  degree stats: min", int(deg.min()), "median", int(np.median(deg)), "max", int(deg.max()))
    print("  observed undirected edges in set:", len(observed_undir))

    rng = np.random.default_rng(SEED)

    print("Loading validation edges (E_val)...")
    ...
    print(f"Training on {DEVICE} (edge-split)...")
    ...
    print("Computing full-train and full-val ROC/PR curves (edge-split)...")
    ...
    plt.title("ROC Curve (edge-split)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, "roc_curve_edge_split.svg"))
    plt.close()
    ...
    plt.title("Precision-Recall Curve (edge-split)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, "prc_curve_edge_split.svg"))
    plt.close()
